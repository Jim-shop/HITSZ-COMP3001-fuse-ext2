...
struct custom_options fs001_options;			 /* 全局选项 */
struct fs001_super super; 
/******************************************************************************
* SECTION: FUSE操作定义
*******************************************************************************/
static struct fuse_operations operations = {
	.init = fs001_init,						 /* mount文件系统 */		
	.destroy = fs001_destroy,				 /* umount文件系统 */
	.mkdir = fs001_mkdir,					 /* 建目录，mkdir */
	.getattr = fs001_getattr,				 /* 获取文件属性，类似stat，必须完成 */
	.readdir = fs001_readdir,				 /* 填充dentrys */
	.mknod = fs001_mknod,					 /* 创建文件，touch相关 */
	.write = NULL,								  	 /* 写入文件 */
	.read = NULL,								  	 /* 读文件 */
	.utimens = fs001_utimens,				 /* 修改时间，忽略，避免touch报错 */
...
void* fs001_init(struct fuse_conn_info * conn_info) {
	...
	/* 下面是一个控制设备的示例 */
	super.fd = ddriver_open(fs001_options.device);
...
int fs001_getattr(const char* path, struct stat * fs001_stat) {
	/* TODO: 解析路径，获取Inode，填充fs001_stat，可参考/fs/simplefs/sfs.c的sfs_getattr()函数实现 */
	return 0;
}
...
int main(int argc, char **argv)
{
	...
	fs001_options.device = strdup("TODO: 这里填写你的ddriver设备路径");
